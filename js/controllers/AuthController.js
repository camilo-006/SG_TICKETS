class AuthController {
    ...
    async handleLogin(email, password) {
        ...
        if (user) {
            ...
            // Verificar si el email existe pero la clave está mal, o si no existe
            const exists = this.model.users.find(u => u.email === email);
            if (exists) {
                this.view.showLoginError("[ ERROR ] Credenciales incorrectas.");
            } else {
                this.view.showLoginError("[ AVISO ] Usuario no encontrado. Por favor, regístrese.");
            }
        }
    }
